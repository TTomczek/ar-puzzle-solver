...
object ModelCreator {
    ...
    fun getModel(entity: PuzzleEntity, augmentedImage: AugmentedImage, anchor: Anchor, viewNodeWindowManager: ViewNode2.WindowManager, materialLoader: MaterialLoader, engine: Engine, showSolution: Boolean, onClick: () -> Unit = {}): AnchorNode? {
        val strategy = modelStrategies.firstOrNull { it.canHandle(entity) }

        return strategy?.createModel(entity, augmentedImage, anchor, viewNodeWindowManager, materialLoader, engine, showSolution, onClick)
    }
}